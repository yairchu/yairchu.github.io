...
Tools like [Protocol Buffers](https://en.wikipedia.org/wiki/Protocol_Buffers) provide a saner solution. You'll only need to declare your data structure once, and it automatically implements both saving and loading. As a bonus, when using statically typed language, it also creates the document's data types for you.
...
Maintaining the help text separately is somewhat like hanging on to a unicycle when already wearing a jetpack. I think that we can leave the unicycle behind :)
...
It means writing code the doesn't do the computation of a single task (like saving a document), but rather describes an essence from which several different processes can be derived.
...
* **General programming language features**: Memory management in retro languages like C requires both allocation and disposal of memory space for data structures. Modern languages with [garbage collection](https://en.wikipedia.org/wiki/Garbage_collection_(computer_science)) or ["smart pointers"](https://en.wikipedia.org/wiki/Smart_pointer) libraries allow us to only describe the allocation and get the disposal for free. This provides more than time saving, as it also solves bugs of memory leaks and corruptions. Similarly type-inference or dynamic typing save us from the task of describing the typing aspects of our code to the compiler.
...
* Declarative code is **less error prone** because there is a single source of truth rather than several variations that need to be kept consistent. No amount of testing could replace the value this brings in eliminating bugs
* The code is more **succint** and **elegant**, as it describes a more abstract **essence** of our program

Cons:

* **Require additional learning**. For example if you want mutually exclusive command line options, instead of adding a straightforward condition you would need to read the library's documentation to find how to specify this
...
* **Performance**: In some cases, like garbage collection, dynamic typing, or taped-based [automatic differentiation](https://en.wikipedia.org/wiki/Automatic_differentiation#Reverse_accumulation), we do pay a hefty performance price for the work saved from us, and hand-crafted undeclarative low-level code may win over the declarative in the benchmarks

Declarative code has an additional, perhaps more important advantage, at least for me: I just like it!

<img src="/images/bugs-chasing-man-on-one-skateboard.jpg" alt="Riding a single skateboard is easier" style="width: 75%"/>

## Notes

The images for this post were all painted by the extremely talented [DALL-E 2](https://openai.com/dall-e-2/) per my requests. Here are the prompts and processes used in creating them:
